...
const ROUND_2_RESULTS = [
  ...
  },
  {
    ...
    teams: ["Team Zeta", "Team Eta", "Team Theta"],
  },
  {
...
function initRound2() {
  ...
  if (track) {
    track.innerHTML = ROUND_2_RESULTS.map(
      (mod) => `
            <div class="module-result-card">
              <div class="module-result-card-inner">
                  <div class="module-result-header">
                      <img src="${mod.icon}" alt="${
        mod.module
      }" class="result-module-icon">
                      <h3 class="result-module-title">${mod.module}</h3>
                  </div>
                  <ul class="qualified-teams-list">
                      ${
                        mod.teams.length
                          ? mod.teams.map((team) => `<li>${team}</li>`).join("")
                          : '<li class="no-results">Results Pending...</li>'
                      }
                  </ul>
              </div>
            </div>
  ...
  let currentIndex = 0;
  const items = ROUND_2_RESULTS.length;
  ...
  function updateCarousel() {
    ...
    const maxIndex = Math.ceil(items / itemsPerView) - 1;
  ...
  const overlay = document.querySelector(".results-modal-overlay");
  const closeBtn = document.querySelector(".results-close-btn");
